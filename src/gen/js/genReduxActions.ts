...
import { renderParamSignature, renderOperationGroup, getParamSignature } from './genOperations'
...
function renderReduxActionBlock(spec: ApiSpec, op: ApiOperation, options: ClientOptions): string {
  ...
  let params = required.map(param => getParamSignature(param, options)).join(', ')
